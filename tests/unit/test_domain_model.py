...
class TestBook:
    ...
    def test_set(self):
        ...
        assert (
            str(sorted(set_of_books))
            == "[<Book West Side Story, book id = 89576>, <Book Harry Potter, book id = 874658>, <Book Hitchhiker's Guide to the Galaxy, book id = 2675376>]"
        )
    ...
    def test_remove_author(self):
        ...
        assert (
            str(book.authors)
            == "[<Author J.R.R. Tolkien, author id = 1>, <Author Neil Gaiman, author id = 2>, <Author Ernest Hemingway, author id = 3>, <Author J.K. Rowling, author id = 4>]"
        )
        ...
        assert (
            str(book.authors)
            == "[<Author J.R.R. Tolkien, author id = 1>, <Author Ernest Hemingway, author id = 3>, <Author J.K. Rowling, author id = 4>]"
        )


class TestReview:
    def test_construction(self):
        # review, user, book, time
        book = Book(2675376, "Harry Potter")
        review_text = "  This book was very enjoyable.   "
        u = User("thor", "Aa1234567")
        date = datetime.today()
        review = Review(review_text, u, book, date)

        assert str(review.book) == "<Book Harry Potter, book id = 2675376>"
        assert str(review.review_text.strip()) == "This book was very enjoyable."
        assert str(review.user == u.user_name)
        assert review.timestamp == date

    def test_attributes_access(self):
        book = Book(2675376, "Harry Potter")
        review = Review(42, book, 3, datetime)
        assert AssertionError

    def test_set_of_reviews(self):
        book1 = Book(2675376, "Harry Potter")
        book2 = Book(874658, "Lord of the Rings")
        review1 = Review("I liked this book", book1, 4, datetime.today())
        review2 = Review("This book was ok", book2, 3, datetime.today())
        review3 = Review("This book was exceptional", book1, 5, datetime.today())
    ...
    def test_wrong_book_object(self):
        ...
        user = User("thor", "Aa1234567")
        review = Review(1, publisher, user, datetime.today())
...
class TestUser:
    ...
    def test_reading_a_book(self):
        ...
        assert (
            str(user.read_books)
            == "[<Book Harry Potter, book id = 874658>, <Book Lord of the Rings, book id = 89576>]"
        )
        assert user.pages_read == 228

    def test_add_to_favourites(self):
        # Checking if user.favourites is empty
        books = [Book(874658, "Harry Potter"), Book(89576, "Lord of the Rings")]
        user = User("Martin", "pw12345")
        assert user.favourites == []

        # Adding Book Objects to user.favourites
        for book in books:
        ...
        # Adding non-Book Objects to user.favourites
        user.add_to_favourites("Book")
        assert (
            str(user.favourites)
            == "[<Book Harry Potter, book id = 874658>, <Book Lord of the Rings, book id = 89576>]"
        )
        # Adding Book Objects twice to user.favourites
        for book in books:
            user.add_to_favourites(book)
        assert (
            str(user.favourites)
    ...
    def test_user_reviews(self):
        ...
        user = User("Martin", "pw12345")
        assert user.reviews == []
        review1 = Review("I liked this book", user, books[0], datetime.today())
        review2 = Review("This book was ok", user, books[1], datetime.today())
        user.add_review(review1)
        user.add_review(review2)
        assert str(user.reviews[0].review_text) == "I liked this book"
        assert user.reviews[0].user == user
        assert str(user.reviews[1].review_text) == "This book was ok"
        assert user.reviews[1].user == user
...
class TestBooksJSONReader:
    def test_read_books_from_file(self, read_books_and_authors):
        ...
        assert (
            str(dataset_of_books[0])
            == "<Book The Switchblade Mamma, book id = 25742454>"
        )
        assert (
            str(dataset_of_books[9])
            == "<Book War Stories, Volume 4, book id = 27036539>"
        )
        assert (
            str(dataset_of_books[19])
            == "<Book D.Gray-man, Vol. 16: Blood & Chains, book id = 18955715>"
        )

    def test_read_books_from_file_and_check_authors(self, read_books_and_authors):
        dataset_of_books = read_books_and_authors
        assert (
            str(dataset_of_books[0].authors[0])
            == "<Author Lindsey Schussman, author id = 8551671>"
        )
        assert (
            str(dataset_of_books[15].authors[0])
            == "<Author Maki Minami, author id = 791996>"
        )
        assert len(dataset_of_books[3].authors) == 2
        assert (
            str(dataset_of_books[3].authors[1])
            == "<Author Chris  Martin, author id = 853385>"
        )

    def test_read_books_from_file_and_check_other_attributes(
        self, read_books_and_authors
    ):
        dataset_of_books = read_books_and_authors
        assert dataset_of_books[2].release_year == 2012
        assert (
            dataset_of_books[19].description
            == "Lenalee is determined to confront a Level 4 Akuma that's out to kill Komui, but her only chance is to reclaim her Innocence and synchronize with it. The Level 4 is not inclined to wait around and pursues its mission even against the best efforts of Lavi and Kanda. It's left to Allen to hold the line, but it soon becomes obvious he has no hope of doing it all by himself!"
        )
